<?php
    require_once("../config.php");
    session_start();
    function getSideBar($type) {
        try {
            $pdo = new PDO(DBCONNSTRING,DBUSER,DBPASS);
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "select ".$type.",count(*) as Updates from travelimage group by ".$type." order by Updates desc";
            $result = $pdo->query($sql);
            $num = 0;
            while ($row = $result->fetch()) {
                if(!$row[$type]) continue;
                if($type === 'CountryCodeISO') echo "<li class=\"list\"><a href=\"browse.php?CountryCodeISO=".$row[$type]."\">".outputCountry($row[$type])."</a></li>";
                else if($type === 'CityCode') echo "<li class=\"list\"><a href=\"browse.php?CityCode=".$row[$type]."\">".outputCity($row[$type])."</a></li>";
                else echo "<li class=\"list\"><a href=\"browse.php?content=".$row[$type]."\">".$row[$type]."</a></li>";
                $num++;
                if($num >= 5) break;
            }
        }
        catch (PDOException $e) {
            die( $e->getMessage() );
        }
    }
    function outputCountry($countryCodeISO) {
        try {
            $pdo = new PDO(DBCONNSTRING,DBUSER,DBPASS);
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "select * from geocountries where ISO = '".$countryCodeISO."'";
            $result = $pdo->query($sql);
            $row = $result->fetch();
            return $row['CountryName'];
        }
        catch (PDOException $e) {
            die( $e->getMessage() );
        }
    }

    function outputCity($cityCode) {
        try {
            $pdo = new PDO(DBCONNSTRING,DBUSER,DBPASS);
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "select * from geocities where GeoNameID = '".$cityCode."'";
            $result = $pdo->query($sql);
            $row = $result->fetch();
            return $row['AsciiName'];
        }
        catch (PDOException $e) {
            die( $e->getMessage() );
        }
    }

    function outputOption() {
        try {
            $pdo = new PDO(DBCONNSTRING,DBUSER,DBPASS);
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "select * from geocountries order by CountryName";
            $result = $pdo->query($sql);
            while ($row = $result->fetch()) {
                echo "<option value=\"".$row['ISO']."\">".$row['CountryName']."</option>";
            }
        }
        catch (PDOException $e) {
            die( $e->getMessage() );
        }
    }


    function getImages($sql) {
        try {
            $pdo = new PDO(DBCONNSTRING,DBUSER,DBPASS);
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $resultAll = $pdo->query($sql);
            $rows = $resultAll->fetchAll();
            $rowCount = count($rows);
            if ( isset($_GET['page']) && $_GET['page'] >1) {
                $curPage = $_GET['page'];
            }else {
                $curPage = 1;
            }
            $limitPage = ($curPage - 1)*16;
            $sql = $sql." limit ".$limitPage.",16";
            $result = $pdo->query($sql);
            $num = 0;
            while ($row = $result->fetch()) {
                outputSingleImage($row);
                $num++;
            }
            $pdo = null;
            return $rowCount;
        }
        catch (PDOException $e) {
            die( $e->getMessage() );
        }
    }

    function outputSingleImage($row) {
        echo "<div class=\"thumbnail\"><br>
                <a href=\"src/details.php?id=".$row['ImageID'].
                "\" class=\"square\" style=\"background-image:url(img/travel-images/medium/".$row['PATH'].")\"></a></div>
            ";
    }



?>
<html>
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="font-awesome-4.7.0/css/font-awesome.css" rel="stylesheet">
    <link href="bootstrap-3.3.7-dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="css/reset.css" rel="stylesheet">
    <link href="css/browse.css" rel="stylesheet">
    <script src="https://apps.bdimg.com/libs/jquery/2.1.4/jquery.min.js"></script>
    <script src="bootstrap-3.3.7-dist/js/bootstrap.min.js"></script>
    <script src="js/filter.js"></script>
    <title>Browse</title>
</head>
<body>
<div class="big-wrapper">
    <div id="topanchor"></div>
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
        <div class="container-fluid">
            <div class="navbar-header">
                <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar-collapse" aria-expanded="false">
                    <span class="sr-only">Toggle navigation</span>
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                </button>
                <a class="navbar-brand" href="#">
                    <img alt="Brand" src="img/icons/logo.png" width="45px" height="45px" style="margin-top: -12px;">
                </a>
            </div>
            <div class="collapse navbar-collapse" id="navbar-collapse">
                <ul class="nav navbar-nav navbar-left">
                    <li><a href="../index.php"><i class="fa fa-home"></i> Home</a></li>
                    <li class="active"><a href=""><i class="fa fa-eye"></i> Browse</a></li>
                    <li><a href="search.php"><i class="fa fa-search"></i> Search</a></li>
                </ul>
                <ul class="nav navbar-nav navbar-right">

                    <?php
                    if(isset($_SESSION['Username'])) echo "
                            <li class=\"dropdown\">
                                    <a href=\"#\" class=\"dropdown-toggle\" data-toggle=\"dropdown\">
                                        <i class=\"fa fa-user-circle\"></i> My&nbsp;account<b class=\"caret\"></b>
                                    </a>
                                    <ul class=\"dropdown-menu\">
                                        <li><a href=\"upload.php\"><i class=\" fa fa-cloud-upload\"></i> Upload</a></li>
                                        <li><a href=\"myphoto.php\"><i class=\"fa fa-camera\"></i> Gallery</a></li>
                                        <li><a href=\"mycollection.php\"><i class=\"fa fa-star\"></i> My&nbsp;collection</a></li>
                                        <li><a href=\"logout.php\"><i class=\"fa fa-sign-out\"></i> Log&nbsp;out</a></li>
                                    </ul>
                             </li>
                            ";
                    else  {
                        echo "
                                <li><a href=\"login.php\"><i class=\"fa fa-sign-in\"></i> Sign&nbsp;in</a></li>
                            ";
                    }
                    ?>
                </ul>
            </div>
        </div>
    </nav>
    <div style="height: 50px"></div>
    <div class = "wrapper">
        <div class="flexbox-left">
            <div class = "countainer">
                <div class="sider search-bar">
                    <div class="sider-title"> Saerch by title</div>
                    <form action="" method="get">
                        <input type="text" name="keyword">
                        <button type="submit" value="submit" ><i class="fa fa-search"></i> </button>
                    </form>
                </div>
                <div class="sider">
                    <div class="sider-title">Hot country</div>
                    <ul>
                        <?php getSideBar('CountryCodeISO'); ?>
                    </ul>
                </div>
                <div class="sider">
                    <div class="sider-title">Hot city</div>
                    <ul>
                        <?php getSideBar('CityCode'); ?>
                    </ul>
                </div>
                <div class="sider">
                    <div class="sider-title">Hot Content</div>
                    <ul>
                        <?php getSideBar('Content'); ?>
                    </ul>
                </div>
            </div>
        </div>
        <div class="flexbox-right">
            <div class="flexbox-right-title">Filter</div>
            <div class="selector">
                <form method="get">
                    <select name="content" class="content-selector">
                        <option value="0">Filter by Content</option>
                        <option value="Scenery">Scenery</option>
                        <option value="City">City</option>
                        <option value="Building">Building</option>
                        <option value="People">People</option>
                        <option value="Animal">Animal</option>
                        <option value="Wonder">Wonder</option>
                        <option value="Other">Other</option>
                    </select>

                    <select name="CountryCodeISO" id="country" onchange="getSelectVal()">
                        <option value="0">Filter by Country</option>
                        <?php outputOption(); ?>
                    </select>

                    <select name="CityCode" id="city">
                        <option value="0">Filter by City</option>
                    </select>
                    <input type="submit" value="Filter" name="continue">
                </form>
            </div>

            <div class="wrapper">
                <?php
                    $cnt = 0;
                    $getList = "?";
                    $sql = "select * from travelimage";
                    if(isset($_GET['keyword'])) {
                        $str = str_replace('　', ' ', $_GET['keyword']);
                        $str = str_replace('  ', ' ', $str);
                        $str = str_replace(' ', '%', $str);
                        if($cnt > 0)  $sql = $sql." and ";
                        else $sql = $sql." where ";
                        $sql = $sql." Title like  '%".$str."%'";
                        $getList = $getList."&keyword=".$_GET['keyword'];
                        $cnt++;
                    }
                    else {

                        if(isset($_GET['content']) and strcmp($_GET['content'],"0")!=0) {
                            if($cnt > 0)  $sql = $sql." and ";
                            else $sql = $sql." where ";
                            $sql = $sql." content = '".$_GET['content']."'";
                            $getList = $getList."&content=".$_GET['content'];
                            $cnt++;
                        }
                        if(isset($_GET['CountryCodeISO']) and strcmp($_GET['CountryCodeISO'],"0")!=0) {
                            if($cnt > 0)  $sql = $sql." and ";
                            else $sql = $sql." where ";
                            $sql = $sql."CountryCodeISO = '".$_GET['CountryCodeISO']."'";
                            $getList = $getList."&CountryCodeISO=".$_GET['CountryCodeISO'];
                            $cnt++;
                        }
                        if(isset($_GET['CityCode']) and strcmp($_GET['CityCode'],"0")!=0) {
                            if($cnt > 0)  $sql = $sql." and ";
                            else $sql = $sql." where ";
                            $sql = $sql."CityCode = '".$_GET['CityCode']."'";
                            $getList = $getList."&CityCode=".$_GET['CityCode'];
                                       $cnt++;
                        }
                    }

                    $rowCount = getImages($sql);
                    $pageCount = ceil(($rowCount + 1)/16);
                    if($pageCount > 5) $pageCount = 5;
                    if ( isset($_GET['page']) && $_GET['page'] >1) {
                        $curPage = $_GET['page'];
                    }else {
                        $curPage = 1;
                    }
                ?>

            </div>
            <div class="page-number">
                <?php
                $prePage = $curPage - 1;
                $nxtPage = $curPage + 1;
                if($curPage > 1) echo "<li><a href=\"".$getList."&page=".$prePage."\"><i class='fa fa-angle-double-left'></i></a></li>";
                for($i = 0; $i < $pageCount; $i++) {
                        $page = $i+1;
                        echo "<li";
                        if($page == $curPage) echo " class = \"active\" ";
                        echo"><a href=\"".$getList."&page=".$page."\">".$page."</a></li>";
                    }
                if($curPage < $pageCount) echo "<li><a href=\"".$getList."&page=".$nxtPage."\"><i class='fa fa-angle-double-right'></i></a></li>";
                ?>
                共<?php echo $pageCount; ?>页,当前在第<?php echo $curPage;?>页
            </div>
    </div>

    <div class="icon-button">
        <a href="#topanchor">
            <button> <i class="fa fa-angle-double-up"></i></button>
        </a>
    </div>
    <div class = "push" ></div>
</div>
    <footer class="footer">
        <p> Copyright © 2020 XZR.All Rights Reserved.</p>
        <p>备案号：沪WEB备19302010085</p>
    </footer>
</body>


</html>